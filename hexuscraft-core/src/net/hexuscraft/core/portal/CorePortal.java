...
public class CorePortal extends MiniPlugin<HexusPlugin> implements PluginMessageListener {
    ...
    public void onEnable() {
        ...
        _coreDatabase._database.registerConsumer(PortalTeleportStaffMessage.CHANNEL_NAME, (_, _, rawMessage) -> _hexusPlugin.runAsync(() -> {
            ...
            _hexusPlugin.getServer().getOnlinePlayers().stream().filter(player -> player.hasPermission(PermissionGroup.TRAINEE.name())).forEach(player -> {
                player.sendMessage(F.fStaff(this, F.fItem(senderName.get()), " sent ", F.fItem(targetName.get()), " to ", F.fItem(message._serverName), "."));
    ...
    public BukkitTask teleportAsync(UUID targetUUID, String serverName) {
        return _hexusPlugin.runAsync(() -> _coreDatabase._database._jedis.publish(PortalTeleportMessage.CHANNEL_NAME, new PortalTeleportMessage(targetUUID, serverName).stringify()));
    }

    public BukkitTask teleportAsync(UUID targetUUID, String serverName, UUID senderUUID) {
        return _hexusPlugin.runAsync(() -> _coreDatabase._database._jedis.publish(PortalTeleportStaffMessage.CHANNEL_NAME, new PortalTeleportStaffMessage(targetUUID, serverName, senderUUID).stringify()));
    }
