...
public final class ServerGroupData {
    ...
    public ServerGroupData(final String name, final Map<String, String> serverGroupData) {
        ...
        _requiredPermission = serverGroupData.getOrDefault("requiredPermission",
                "PLAYER");
        _minPort = Integer.parseInt(serverGroupData.getOrDefault("minPort",
                "0"));
        _maxPort = Integer.parseInt(serverGroupData.getOrDefault("maxPort",
                "0"));
        _totalServers = Integer.parseInt(serverGroupData.getOrDefault("totalServers",
                "0"));
        _joinableServers = Integer.parseInt(serverGroupData.getOrDefault("joinableServers",
                "0"));
        _plugin = serverGroupData.getOrDefault("plugin",
                "");
        _worldZip = serverGroupData.getOrDefault("worldZip",
                "");
        _ram = Integer.parseInt(serverGroupData.getOrDefault("ram",
                "512"));
        _capacity = Integer.parseInt(serverGroupData.getOrDefault("capacity",
                "20"));
        _worldEdit = Boolean.parseBoolean(serverGroupData.getOrDefault("worldEdit",
                "FALSE"));
        _timeoutMillis = Integer.parseInt(serverGroupData.getOrDefault("timeoutMillis",
                "30000"));
        _games = serverGroupData.getOrDefault("games",
                        "")
                .split(",");
        _hostUniqueId =
                UUID.fromString(serverGroupData.getOrDefault("hostUniqueId",
    ...
    public Map<String, String> toMap() {
        ...
        map.put("requiredPermission",
                _requiredPermission);
        map.put("minPort",
                Integer.toString(_minPort));
        map.put("maxPort",
                Integer.toString(_maxPort));
        map.put("totalServers",
                Integer.toString(_totalServers));
        map.put("joinableServers",
                Integer.toString(_joinableServers));
        map.put("plugin",
                _plugin);
        map.put("worldZip",
                _worldZip);
        map.put("ram",
                Integer.toString(_ram));
        map.put("capacity",
                Integer.toString(_capacity));
        map.put("worldEdit",
                Boolean.toString(_worldEdit));
        map.put("timeoutMillis",
                Integer.toString(_timeoutMillis));
        map.put("games",
                String.join(",",
                        _games));
        map.put("hostUniqueId",
                _hostUniqueId.toString());
        return map;
    }

    public void update(final UnifiedJedis jedis) {
        jedis.hset(ServerQueries.SERVERGROUP(_name),
                toMap());
    }

    public ServerData[] getServers(final UnifiedJedis jedis) {
        return ServerQueries.getServers(jedis,
                this);
    }
