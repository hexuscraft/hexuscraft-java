...
public final class CoreNpc extends MiniPlugin<HexusPlugin> {
	...
	public void createEntity(World world, double x, double y, double z, float yaw, float pitch, String[] data) {
		...
		switch (data[0]) {
			case "PLAYER_SERVERS" -> {
				...
				NBTTagCompound villagerNBT = UtilEntity.getNBTTagCompound(villager);
				villagerNBT.setByte("NoAI", (byte) 1);
				villagerNBT.setByte("Silent", (byte) 1);
				villagerNBT.setByte("Invulnerable", (byte) 1);
				UtilEntity.saveNBTTagCompound(villager, villagerNBT);
			...
			case "MISSIONS" -> {
				...
				ArmorStand armorStand = world.spawn(location, ArmorStand.class);
				spawnedEntities.add(armorStand);
				armorStand.setCustomName(C.cGreen + C.fBold + "Missions");
				armorStand.setCustomNameVisible(true);
				armorStand.setGravity(false);
				armorStand.setMarker(true);
				armorStand.teleport(villager.getEyeLocation().add(new Vector(0, 0.25, 0)));
			...
			case "AMPLIFIERS" -> {
				...
				ArmorStand armorStand = world.spawn(location, ArmorStand.class);
				spawnedEntities.add(armorStand);
				armorStand.setCustomName(C.cGreen + C.fBold + "Amplifiers");
				armorStand.setCustomNameVisible(true);
				armorStand.setGravity(false);
				armorStand.setMarker(true);
				armorStand.teleport(villager.getEyeLocation().add(new Vector(0, 0.25, 0)));
				...
				ArmorStand amplifierInfo = world.spawn(location, ArmorStand.class);
				spawnedEntities.add(amplifierInfo);
				amplifierInfo.setCustomName(C.cGray + "None Active");
				amplifierInfo.setCustomNameVisible(true);
				amplifierInfo.setGravity(false);
				amplifierInfo.setMarker(true);
				amplifierInfo.teleport(villager.getEyeLocation().clone().add(new Vector(0, 0.5, 0)));

				NBTTagCompound amplifierInfoNBT = UtilEntity.getNBTTagCompound(amplifierInfo);
				amplifierInfoNBT.setByte("Invisible", (byte) 1);
				amplifierInfoNBT.setByte("NoGravity", (byte) 1);
			...
			case "LOBBY" -> {
				...
				ArmorStand armorStand = world.spawn(location, ArmorStand.class);
				spawnedEntities.add(armorStand);
				armorStand.setCustomName(C.cGreen + C.fBold + "Return to Lobby");
				armorStand.setCustomNameVisible(true);
				armorStand.setGravity(false);
				armorStand.setMarker(true);
				armorStand.teleport(villager.getEyeLocation().add(new Vector(0, 0.25, 0)));
			...
			case "GAME" -> {
				...
				gameName.setCustomName(C.cGreen + C.fBold + (data[1].equals("CLANS") ? "Clans" : GameType.valueOf(data[1])._name));
	...
	public void refreshNPCs(final World world) {
		...
		try (Scanner scanner = new Scanner(Path.of(world.getWorldFolder().getPath(), "_npcs.dat").toFile())) {
	...
	boolean handleInteractNPC(final Player player, final Entity npc) {
		if (!npc.hasMetadata("NPC")) return false;
		String[] data = (String[]) npc.getMetadata("NPC").getFirst().value();
		player.sendMessage(F.fMain(this, "Metadata: ", F.fItem(data)));
		return true;
	}
	...
	void onEntityDamageByEntity(final EntityDamageByEntityEvent event) {
		...
		if (!handleInteractNPC(player, event.getEntity())) return;
		event.setCancelled(true);
	}

	@EventHandler
	void onPlayerInteractAtEntity(final PlayerInteractAtEntityEvent event) {
		if (!handleInteractNPC(event.getPlayer(), event.getRightClicked())) return;
		event.setCancelled(true);
	}
	...
	public record NpcData(String[] _data) {

		public static NpcData fromString(String jsonString) {
			final JSONArray jsonArray = new JSONArray(jsonString);
			final String[] data = new String[jsonArray.length()];
			for (int i = 0; i < jsonArray.length(); i++) data[i] = jsonArray.getString(i);
		...
		public String toString() {
			return new JSONArray(_data).toString();
		}
