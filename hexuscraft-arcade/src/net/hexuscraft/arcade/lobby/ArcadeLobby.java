...
public final class ArcadeLobby extends MiniPlugin<Arcade> {
    ...
    public void onPlayerJoin(final PlayerJoinEvent event) {
        if (_arcadeManager.getGameState() == GameState.IN_PROGRESS) return;
    ...
    public void onEntityDamageByEntity(final EntityDamageByEntityEvent event) {
        ...
        if (!player.getWorld()
                .equals(_hexusPlugin.getServer()
                        .getWorlds()
                        .getFirst())) return;

        if (event.getDamager() instanceof final Player damager && damager.getGameMode()
