...
public final class ItemSearch {
    ...
    public static Material[] itemSearch(final String searchName) {
        ...
        final Material[] matches = Arrays.stream(Material.values()).filter(ItemSearch::isMaterialAnItem).filter(material -> material.name().toLowerCase().contains(searchName.toLowerCase())).toArray(Material[]::new);
    ...
    public static Material[] itemSearch(final String searchName, final CommandSender sender, final Predicate<Material[]> shouldSendMatches) {
        final Material[] matches = itemSearch(searchName);
        if (shouldSendMatches.test(matches))
            sender.sendMessage(F.fMain("Item Search", F.fMatches(Arrays.stream(matches).map(Material::name).toArray(String[]::new), searchName)));
        return matches;
    }
