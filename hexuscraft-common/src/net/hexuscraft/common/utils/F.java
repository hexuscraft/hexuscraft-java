...
public class F {
	...
	public static String[] fItem(Map<String, String> map) {
		List<String> strings = new ArrayList<>();
		map.forEach((key, value) -> strings.add(C.cYellow + key + C.cGray + " - " + C.cYellow + value));
	...
	public static String fMatches(String[] matches, String searchName) {
		return F.fItem(matches.length + (matches.length == 1 ? " Match" : " Matches")) + C.cGray + " for " + F.fItem(searchName) + C.cGray + (matches.length == 0 ? "." : ": " + F.fItem(matches));
	}
	...
	public static String fChat(int level, PermissionGroup permissionGroup) {
		return C.cGray + level + " " + fPermissionGroup(permissionGroup, true, true) + C.cYellow + " %s" + C.fReset + " " + "%s";
	}
	...
	public static String fPunish(PunishData punishData) {
		switch (punishData._type) {
			case WARNING -> {
				return C.cRed + C.fBold + "You received a warning\n" + C.cWhite + punishData._reason + C.cDGreen + "\nUnfairly punished? Appeal at " + C.cGreen + "www.hexuscraft.net\n\n" + C.cDGray + punishData._uuid.toString();
			}
			case KICK -> {
			...
			case MUTE -> {
				return C.cRed + C.fBold + "You are muted for " + F.fItem(F.fTime(punishData._length)) + "\n" + C.cWhite + punishData._reason + C.cDGreen + "\nUnfairly punished? Appeal at " + C.cGreen + "www.hexuscraft.net\n\n" + C.cDGray + punishData._uuid.toString();
			}
		}
	...
	public static String fCheat(String playerName, CheatSeverity severity, String reason) {
		return F.fStaff(severity._color) + F.fMain("AC", F.fItem(playerName), " failed ", severity._color + reason + C.cGray, ".");
	}

	public static String fCheat(String playerName, CheatSeverity severity, String reason, String serverName) {
		return F.fStaff(severity._color) + F.fMain("AC", F.fItem(playerName), " failed ", severity._color + reason + C.cGray, " in ", serverName, ".");
	}
	...
	public static String fWelcomeMessage(String playerName) {
		return String.join("§r\n", " ", " §b§lWelcome " + playerName + " to Hexuscraft!", "  §7A mini-game server inspired by the legacy Mineplex Network", "   §7Type §e/help§7 to see available commands ", " ", "  §7We are open source! Contribute to help improve our server", "   §e§nhttps://github.com/hexuscraft", " ");
	}
