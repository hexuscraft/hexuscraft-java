...
public final class CoreChat extends MiniPlugin<HexusPlugin> {
	...
	final String CHANNEL_ANNOUNCEMENT = "ChatAnnouncement";
	...
	public void onEnable() {
		...
		_coreCommand.register(new CommandSupportResponse(this, _corePermission, _coreDatabase, _corePortal, _coreActionBar));
		...
		_coreDatabase._database.registerConsumer(ChatAnnouncementMessage.CHANNEL_NAME, (_, _, rawMessage) -> {
			...
			_hexusPlugin.runAsync(() -> {
				final AtomicReference<String> senderName = new AtomicReference<>("<UNKNOWN>");
				try {
					senderName.set(PlayerSearch.offlinePlayerSearch(parsedMessage._senderUniqueId()).getName());
				} catch (URISyntaxException | IOException ex) {
					logWarning(ex);
				}

				_hexusPlugin.getServer().getOnlinePlayers().stream().filter(player -> player.hasPermission(PermissionGroup.ADMINISTRATOR.name())).forEach(player -> player.sendMessage(F.fStaff(this, F.fItem(senderName.get()), " sent an announcement to ", F.fPermissionGroup(parsedMessage._permissionGroup()), ".")));
			});

			_hexusPlugin.getServer().getOnlinePlayers().stream().filter(player -> player.hasPermission(parsedMessage._permissionGroup().name())).forEach(player -> {
		...
		_coreDatabase._database.registerConsumer(ChatSupportMessage.CHANNEL_NAME, (_, _, rawMessage) -> {
			...
			_hexusPlugin.getServer().getOnlinePlayers().stream().filter(player -> player.getUniqueId().equals(messageData._senderUniqueId()) || player.hasPermission(PERM.COMMAND_SUPPORT_STAFF.name())).forEach(player -> {
				player.sendMessage(C.cPurple + messageData._senderServerName() + " " + F.fPermissionGroup(PermissionGroup.getGroupWithHighestWeight(messageData._senderPermissionGroups())) + " " + messageData._senderName() + C.cPurple + " " + messageData._message());
		...
		_coreDatabase._database.registerConsumer(ChatSupportResponseSentMessage.CHANNEL_NAME, (_, _, rawMessage) -> {
			...
			PermissionGroup[] targetPermissionGroups = _corePermission._permissionProfiles.containsKey(target) ? _corePermission._permissionProfiles.get(target)._groups() : new PermissionGroup[0];

			_hexusPlugin.runAsync(() -> _coreDatabase._database._jedis.publish(ChatSupportResponseReceivedMessage.CHANNEL_NAME, new ChatSupportResponseReceivedMessage(parsedMessage._uuid(), parsedMessage._senderUniqueId(), parsedMessage._senderName(), parsedMessage._senderServerName(), parsedMessage._senderPermissionGroups(), target.getUniqueId(), target.getName(), _corePortal._serverName, targetPermissionGroups, parsedMessage._message()).toString()));
		});

		_coreDatabase._database.registerConsumer(ChatSupportResponseReceivedMessage.CHANNEL_NAME, (_, _, rawMessage) -> {
			ChatSupportResponseReceivedMessage parsedMessage = ChatSupportResponseReceivedMessage.fromString(rawMessage);
			_hexusPlugin.getServer().getOnlinePlayers().stream().filter(player -> player.getUniqueId().equals(parsedMessage._senderUniqueId()) || player.getUniqueId().equals(parsedMessage._targetUniqueId()) || player.hasPermission(PERM.COMMAND_SUPPORT_STAFF.name())).forEach(player -> {
				if (player.getUniqueId().equals(parsedMessage._senderUniqueId())) {
					player.sendMessage(C.cDPurple + "-> " + C.cPurple + parsedMessage._targetServerName() + " " + F.fPermissionGroup(PermissionGroup.getGroupWithHighestWeight(parsedMessage._targetPermissionGroups())) + " " + parsedMessage._targetName() + " " + C.cPurple + parsedMessage._message());
				} else if (player.getUniqueId().equals(parsedMessage._targetUniqueId())) {
					player.sendMessage(C.cDPurple + "<- " + C.cPurple + parsedMessage._senderServerName() + " " + F.fPermissionGroup(PermissionGroup.getGroupWithHighestWeight(parsedMessage._senderPermissionGroups())) + " " + parsedMessage._senderName() + " " + C.cPurple + parsedMessage._message());
				} else {
					player.sendMessage(C.cPurple + parsedMessage._senderServerName() + " " + F.fPermissionGroup(PermissionGroup.getGroupWithHighestWeight(parsedMessage._senderPermissionGroups())) + " " + parsedMessage._senderName() + C.cDPurple + " -> " + C.cPurple + parsedMessage._targetServerName() + " " + F.fPermissionGroup(PermissionGroup.getGroupWithHighestWeight(parsedMessage._targetPermissionGroups())) + " " + parsedMessage._targetName() + " " + C.cPurple + parsedMessage._message());
				}
	...
	public void setMuted(boolean toggle) {
		_chatMuted = toggle;
		_hexusPlugin.getServer().broadcastMessage(F.fMain(this, toggle ? F.fError("The chat is now silenced.") : F.fSuccess("The chat is no longer silenced.")));
	}
	...
	void onAsyncPlayerChat(AsyncPlayerChatEvent event) {
		...
		if (player.hasPermission(PERM.CHAT_PREFIX.name())) {
			event.setFormat(F.fChat(0, PermissionGroup.getGroupWithHighestWeight(_corePermission._permissionProfiles.get(player)._groups())));
		} else {
